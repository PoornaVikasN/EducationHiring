      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0"
            ...
            <Shield className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: '#16a34a' }} />
          </div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-black" style={{ color: '#0d1b2a' }}>{page?.title ?? 'Privacy Policy'}</h1>
        </div>
        <p className="text-sm mb-8 sm:mb-12" style={{ color: '#64748b' }}>{page?.lastUpdatedLabel ?? 'Last updated: July 2026'}</p>

        {page ? (
          <div className="legal-body space-y-8 sm:space-y-10" style={{ color: '#334155', lineHeight: 1.8, fontSize: '0.9375rem' }}>
            {page.sections.map((s) => (
