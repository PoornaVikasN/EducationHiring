...
function LoginForm() {
  ...
  const [googleRole, setGoogleRole] = useState<Role>(Role.JOB_SEEKER);
    ...
    <>
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          {[Role.JOB_SEEKER, Role.RECRUITER].map((r) => (
            <Button
              key={r}
              type="button"
              variant={googleRole === r ? 'default' : 'outline'}
              size="sm"
              className="flex-1 text-xs"
              onClick={() => setGoogleRole(r)}
            >
              {r === Role.JOB_SEEKER ? 'Teacher' : 'School'}
            </Button>
          ))}
        </div>
        <Button
